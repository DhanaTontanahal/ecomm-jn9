...
export default function POS() {
    ...
    const setLinePrice = (id, val) =>
        setCart((prev) =>
            prev.map((x) =>
                x.id === id ? { ...x, price: Number(val) || 0 } : x
            )
        );
    ...
    const onEnterSearch = (e) => {
        if (e.key === "Enter") fetchProducts(catId);
    };
    const chooseCat = (c) => {
        setCatId(c?.id || "ALL");
        setCatSheet(false);
        fetchProducts(c?.id || "ALL");
    };
            ...
            const lines = problems
                .map(({ it, reason, stock }) => {
                    ...
                    if (reason === "insufficient")
                        return `• ${label}: Only ${stock} left, requested ${it.qty}`;
                    if (reason === "low")
                        return `• ${label}: Low stock (${stock} available). Min required is 5`;
                    if (reason === "missing")
        ...
        try {
            ...
            const payload = {
                ...
                cashier: user
                    ...
                    : null,
                customer: {
                    name: (custName || "Guest").trim(),
                    phone: phoneDigits(custPhone),
                },
                ...
                <CartHead>
                    ...
                    >
                        ...
                        {selectedCat ? (
                            <Pill>
                                Category:{" "}
                                {selectedCat.displayName || selectedCat.title}
                            </Pill>
                        ) : null}
                    </div>
                    {cart.length > 0 && (
                                    ...
                                    >
                                        ...
                                        >
                                            <QtyCtrl>
                                                ...
                                                <input
                                                    value={it.qty}
                                                    onChange={(e) =>
                                                        setLineQty(it.id, e.target.value)
                                                    }
                                                    style={{
                                                        width: 48,
                                                        textAlign: "center",
                                        ...
                                        >
                                            ...
                                            <input
                                                value={it.price}
                                                onChange={(e) =>
                                                    setLinePrice(it.id, e.target.value)
                                                }
                                                style={{
                                                    width: 92,
                                                    textAlign: "right",
                ...
                <div>
                    ...
                    <Footer>
                        ...
                        <Two style={{ marginTop: 8 }}>
                            ...
                            >
                                <Secondary onClick={() => window.print()}>
                                    <FiPrinter /> Print
                                </Secondary>
                                <Secondary
                                    onClick={() =>
                        ...
                        <CTA
                            disabled={
                                !cart.length || placing || !phoneOk || !upiOk
                            }
                            onClick={placeOrder}
                            style={{ marginTop: 8 }}
                        >
