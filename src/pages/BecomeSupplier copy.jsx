...
export default function BecomeSupplier() {
    ...
    const submit = async (e) => {
        e.preventDefault();
        if (!user) return nav("/login", { state: { next: "/become-supplier" } });
        if (!form.companyName || !form.contactName || !form.phone) return alert("Please fill required fields.");
        try {
            setSaving(true);
            ...
            <Wrap>
                <Card as="form" onSubmit={submit}>
                    <Row>
                        ...
                        <Input name="companyName" value={form.companyName} onChange={onChange} placeholder="e.g., Soami Organic Foods" />
                    </Row>
                    <Row>
                        <Label>Contact Person *</Label>
                        <Input name="contactName" value={form.contactName} onChange={onChange} placeholder="Your full name" />
                    </Row>
                    <Row>
                        <Label>Phone *</Label>
                        <Input name="phone" value={form.phone} onChange={onChange} placeholder="10-digit mobile" inputMode="tel" />
                    </Row>
                    <Row>
                        <Label>Email</Label>
                        <Input name="email" value={form.email} onChange={onChange} placeholder="name@company.com" />
                    </Row>
                    <Row>
                        <Label>Product Category</Label>
                        <Input name="category" value={form.category} onChange={onChange} placeholder="e.g., Cold-pressed oils, Millets" />
                    </Row>
                    <Row>
                    ...
                    <Row>
                        <Label>Notes</Label>
                        <Textarea name="message" value={form.message} onChange={onChange} placeholder="Tell us about your products, certifications, etc." />
                    </Row>

                    <Actions>
                        <Primary type="submit" disabled={saving}>{saving ? "Submitting…" : "Submit Request"}</Primary>
                        <Secondary type="button" onClick={() => nav("/accounts")}>Cancel</Secondary>
                    </Actions>
                    <Note>Submitted under your account. Our team will contact you soon.</Note>
                </Card>
            </Wrap>
        </Page>
    );
