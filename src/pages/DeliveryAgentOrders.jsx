...
const List = styled.div`
  ...
  padding-bottom: 80px; /* breathing room at bottom for mobile */
`;
...
function getOrderCoords(order) {
    ...
    const geo =
        order.shippingGeo ||
        order.shippingLocation?.geo ||
        order.shippingLocationGeo;

    if (geo && typeof geo.latitude === "number" && typeof geo.longitude === "number") {
        return { lat: geo.latitude, lng: geo.longitude };
    }

    // Optional fallback if you ever store numeric fields
    if (typeof order.shippingLat === "number" && typeof order.shippingLng === "number") {
...
function openNavigation(order) {
    ...
    if (coords) {
        // Open Google Maps directions to coordinates
        const url = `https://www.google.com/maps/dir/?api=1&destination=${coords.lat},${coords.lng}`;
        window.open(url, "_blank", "noopener,noreferrer");
        return;
    }

    // Fallback: use text address if no coordinates available
    if (order.shippingAddress) {
        ...
        alert("No location info available for this order.");
    }
}
                        ...
                        {filtered.map((o) => {
                            ...
                            return (
                                <Card key={o.id}>
                                    ...
                                    <ActionsCol>
                                        {/* NEW: Navigate button */}
                                        <SecondaryBtn onClick={() => openNavigation(o)}>
                                            <FiMapPin /> Navigate to delivery
                                        </SecondaryBtn>
