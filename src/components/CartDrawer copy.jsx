...
export default function CartDrawer({ selectedAddress }) {
  ...
  const handleCheckout = async () => {
    ...
    if (paymentMode === "UPI") {
      const v = (upiId || "").trim();
      if (!v) { setUpiError("UPI transaction ID is required."); return; }
      setUpiError("");
    }

    // 🔹 NEW: Build delivery block with coords from selectedAddress
    const deliveryBlock =
      selectedAddress
        ? {
    ...
    try {
      ...
      const payload = {
        ...
        items: cart.items.map(x => ({
          id: x.id, title: x.title, price: Number(x.price || 0), qty: x.qty,
          imageUrl: x.imageUrl || null, mrp: Number(x.mrp || 0) || null,
          sizeLabel: x.sizeLabel || null, subtitle: x.subtitle || null,
        ...
        // 🔹 NEW: only attach delivery if we actually have something
      ...
      // Use safe email (same one used in payload.customer)
      await notifyAdminsNewOrder({ email: payload.customer.email, total });

      cart.clear();
      cart.closeCart();
      setDoneText(paymentMode === "UPI"
