...
const Select = styled.select`
  ...
  /* make native control adopt dark color palette where supported */
  color-scheme: dark;

  &:focus{ outline:none; box-shadow:0 0 0 3px ${COLORS.ring} };

  /* option popup colors (supported by most modern browsers) */
  option {
    background: #121a2b;       /* dark panel */
    color: ${COLORS.text};     /* visible text */
  }

  /* placeholder style if you add a disabled option */
  &:invalid {
...
function emptyProduct(catId, catSlug) {
    return {
        ...
        subtitle: "",
        imageUrl: "",
        imagePath: "", // NEW: keep storage path to clean up later
        sizeLabel: "1L",
        mrp: 0,
        price: 0,
        cashbackAmount: 0,
...
export default function AdminProductManager() {
    ...
    useEffect(() => {
        // (async () => {
        //     const cSnap = await getDocs(query(collection(db, "productCategories"), where("active", "==", true), orderBy("order", "asc")));
        //     const list = cSnap.docs.map(d => ({ id: d.id, ...d.data() }));
        //     setCats(list);
        //     if (!catId && list[0]) { setCatId(list[0].id); setCatSlug(list[0].slug); }
        // })();
    ...
    useEffect(() => {
        if (!catId) return;
        (async () => {
            const qy = query(collection(db, "products"), where("categoryId", "==", catId), orderBy("order", "asc"));
            const s = await getDocs(qy);
            setRows(s.docs.map(d => ({ id: d.id, ...d.data() })));
        })();
    }, [catId]);
        ...
        <Wrap>
            <Head>
                ...
                <Row>
                    ...
                    {/* <Button onClick={() => setEditing(emptyProduct(catId, catSlug))}>
                        <FiPlus /> Add Product
                    </Button> */}
                ...
                <tbody>
                    {rows.map(p => (
                        <tr key={p.id}>
                            <td>
                                <Row>
                                    {p.imageUrl ? <img src={p.imageUrl} alt="" /> : <div style={{ width: 56, height: 56, background: "#222", borderRadius: 8 }} />}
                                    <div>
                            ...
                            <td>
                                ...
                                <div style={{ fontSize: 12, color: COLORS.subtext }}>Cashback: ₹ {p.cashbackAmount ?? Math.round((p.price || 0) * 0.10)}</div>
                            </td>
                            <td>
...

            {
                catId && !loadingRows && rows.length === 0 && (
                    <Notice>No products added in <b>{selectedCat?.displayName || selectedCat?.title}</b> yet.</Notice>
                )
            }
                ...
                catId && !loadingRows && rows.length > 0 && (
                    <Table>
                        ...
                        <tbody>
                            {rows.map(p => (
                                <tr key={p.id}>
                                    {/* … your existing row markup unchanged … */}
                                </tr>
                            ))}
                        </tbody>
                    </Table>
