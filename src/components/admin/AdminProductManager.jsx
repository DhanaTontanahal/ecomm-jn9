...
import AdminBulkProductUploader from "./AdminBulkProductUploader";
...
function useConfirm() {
  ...
  const ConfirmModal = state.open ? (
    <Blocker role="dialog" aria-modal="true" onClick={() => close(false)}>
      <div
      ...
      >
        <div style={{ padding: 16, borderBottom: `1px solid ${COLORS.glassBorder}`, background: COLORS.glassHeader }}>
          <h3 style={{ margin: 0, fontSize: 18 }}>{state.title || "Are you sure?"}</h3>
        </div>
        <div style={{ padding: 16, color: COLORS.subtext, lineHeight: 1.5 }}>
          {state.body || "This action cannot be undone."}
        </div>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", padding: 12, borderTop: `1px solid ${COLORS.glassBorder}` }}>
          <SmallBtn onClick={() => close(false)}>{state.cancelText}</SmallBtn>
          <Button
            $danger={state.danger}
            onClick={() => close(true)}
          >
...
export default function AdminProductManager() {
  ...
  async function removeProduct(prod) {
    ...
    const ok = await confirm({
      ...
      body: `This will permanently delete "${prod.title || "the product"}" and its image from storage (if any).`,
      confirmText: "Delete",
      cancelText: "Cancel",
      danger: true,
    });
  ...
  return (
    <Wrap>
      <Head>
        ...
        <Row>
          ...
          <Button
            ...
            disabled={!catId}
            title={catId ? "Bulk upload default products" : "Pick a category first"}
          >
            <FiUploadCloud /> Bulk Upload Defaults
          </Button>

          <Button
            onClick={() => setOpenBulk(true)}
            disabled={!catId}
            title={catId ? "Select from existing products" : "Pick a category first"}
          >
            <FiPackage /> Select From Existing
          </Button>
        </Row>
      </Head>
            ...
            {rows.map(p => (
              <tr key={p.id}>
                <td>
                  <Row>
                    ...
                    <div>
                      ...
                      {p.sku && (
                        <>
                          <div style={{ fontSize: 12, marginTop: 4 }} className="mono">SKUs: {p.sku}</div>
                        </>
                      )}
                      {true && (
                        <>
                          <p>Zoho inventoryId:{p?.zohoItemId}</p>
                        </>
                      )}
                    </div>
                  </Row>
                ...
                <td>
                  ...
                  <div>
                    <span className="mono">MRP</span>: ₹ {p.mrp}
                    {p.sizeLabel ? <span className="mono"> / {p.sizeLabel}</span> : null}
                  </div>
                  <div>
                    <b>Price</b>: ₹ {p.price}
                    {p.sizeLabel ? <span className="mono"> / {p.sizeLabel}</span> : null}
                ...
                <td>
                  ...
                  <div style={{ marginTop: 6 }}>
                    Current: <b>{p.stock ?? 0}</b> {p.sizeLabel ? <span className="mono">(*{p.sizeLabel})</span> : null}
                  </div>
      ...
      {/* Bulk uploader / existing picker modal */}
      <AdminBulkProductUploader
        open={openBulk}
        onClose={() => setOpenBulk(false)}
        defaultCategoryId={catId}
        defaultCategorySlug={catSlug}
        onComplete={async () => {
          // refresh rows after upload/copy
          if (!catId) return;
          setLoadingRows(true);
          try {
            const qy = query(
...
export function ProductEditor({ product, onSave, onClose }) {
  ...
  return (
    <div style={{
      ...
      <Row style={{ marginBottom: 10 }}>
        <div style={{ flex: 1 }}>
          <Label>MRP {form.sizeLabel ? `(/ ${form.sizeLabel})` : ""}</Label>
          <Input type="number" value={form.mrp || 0} onChange={e => set("mrp", Number(e.target.value))} />
        </div>
        <div style={{ flex: 1 }}>
          <Label>Sale Price {form.sizeLabel ? `(/ ${form.sizeLabel})` : ""}</Label>
        ...
        <div style={{ flex: 1 }}>
          <Label>Enter n: Stock {form.sizeLabel ? `(in order of n * ${form.sizeLabel})` : ""}</Label>
