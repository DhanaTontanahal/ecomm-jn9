...
const Bar = styled.div`
  ...
  grid-template-columns: auto 1fr auto; /* brand left, nav center, right tools */
  column-gap: 12px;
...
//   font-weight:900; letter-spacing:.6px; color:${TOK.brand};
//   font-size: clamp(14px, 2.2vw, 18px);
//   user-select:none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
//   max-width: 42vw;
//   @media (min-width:981px){ max-width: 360px; }
// `;
...
const DeskBody = styled.div`
  padding: 14px; display: grid; gap: 12px; overflow: auto;
`;
...
const DeskName = styled.div`
  font-weight: 900; color: ${TOK.text};
`;

const DeskEmail = styled.div`
...
const BrandText = styled.div`
  ...
  /* Slightly larger on small screens, but still capped on desktop */
  font-size: clamp(4px, 2.5vw, 7.5px);
  user-select:none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  /* Let the title use whatever space is left after buttons, logo and padding */
  max-width: 40vw; /* default */
  @media (max-width:980px){
    /*  menu(32) + logo(28) + 2 icons(2×32) + gaps/padding(~56) ≈ 180px */
...
`;
const MobileAuto = styled.div`
  max-width:${TOK.maxW}; margin: 6px auto 10px; padding: 0 12px 12px;
`;
...
export default function HeaderWithCarousel() {
  ...
  }, [catsOpen, accountOpen, addrFormOpen, moreOpen]);
  ...
  const handleUserClick = () => {
    ...
    if (isMobile) {
      setAccountOpen(true);    // existing mobile sheet
    } else {
      setAccountDesktopOpen(true); // new desktop drawer
    }
  };
  ...
  const doLogout = async () => {
    try {
      // Prefer your AuthProvider’s logout if present
  ...
  const doSearch = async (text) => {
    ...
    // Preferred: prefix search on `nameLower`
    try {
    ...
    // Fallback: fetch a small window and filter client-side
    const ref = collection(db, "products");
  ...
  const goToSearchPage = (term) => {
    ...
    if (!text) return;
    // Your app has /menu — route with a query param
    // nav(`/menu?search=${encodeURIComponent(text)}`);
  ...
  const onSelectResult = (r) => {
    // If you later add a product details page, switch this navigation.
    // For now, go to /menu?search=name to keep it working with your routing.
    // nav(`/menu?search=${encodeURIComponent(r.name)}`);
    nav(`/search?q=${encodeURIComponent(text)}`);
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setSelIndex(i => (i + 1) % results.length);
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      setSelIndex(i => (i - 1 + results.length) % results.length);
    } else if (e.key === "Enter") {
      e.preventDefault();
      if (selIndex >= 0) onSelectResult(results[selIndex]);
      else goToSearchPage();
    }
  };
      ...
      <Header>
        <Bar>
          ...
          <BrandWrap>
            ...
            <LogoCircle>{brand.initials}</LogoCircle>
          ...
          <Nav aria-label="Primary">
            <NavList>
              ...
              {/* More dropdown when extra categories exist */}
              {tailCats.length > 0 && (
                <NavItem ref={moreRef}>
                  <MoreWrap>
                    <button
                ...
                addresses.map(a => (
                  <Card key={a.id}>
                    <div>
                      ...
                      <AddressLine>
                        {a.line1}{a.line2 ? `, ${a.line2}` : ""}{a.landmark ? `, ${a.landmark}` : ""}, {a.city} - {a.pincode}
                      </AddressLine>
                    </div>
          ...
          <Section>
            <Form onSubmit={async (e) => {
              ...
              const payload = {
                ...
                city: form[5].value.trim(),
                pincode: form[6].value.trim(),
                tag: "Home",
