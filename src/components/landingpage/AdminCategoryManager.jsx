...
export default function AdminCategoryManager() {
    const [zohoItemId, setZohoItemId] = useState(null);
    ...
    const handleSave = async () => {
        try {
            ...
            const payload = {
                ...
                zohoItemId: zohoItemId || null,  // keep whatever we already have
            };
            ...
            // 🔁 Try syncing to Zoho (non-blocking for Firestore save)
            try {
                const resp = await fetch(`${API_BASE}/zoho/sync-category`, {
                    method: "POST",
                ...
                if (json?.ok && json.zohoItemId) {
                    // save mapping in Firestore & local state
                    await updateDoc(doc(db, "productCategories", docId), {
                        zohoItemId: json.zohoItemId,
                    });
                    setZohoItemId(json.zohoItemId);
                } else if (!resp.ok) {
                    console.error("Zoho sync failed:", json || (await resp.text()));
                }
            } catch (err) {
                console.error("Zoho sync error:", err);
                // you can plug in a toast here if you want to notify UI
            }
    ...
    const handleEdit = (it) => {
        ...
        setZohoItemId(it.zohoItemId || null);      // 👈 NEW
        if (fileRef.current) fileRef.current.value = "";
