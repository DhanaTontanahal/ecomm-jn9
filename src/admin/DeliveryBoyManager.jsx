// src/pages/DeliveryBoyManager.jsx
...
export default function DeliveryBoyManager() {
    ...
    async function add() {
        if (!name.trim() || !phone.trim() || !passkey.trim()) {
            return alert("Enter name, phone & passkey");
        }
        await addDoc(collection(db, "deliveryBoys"), {
            name: name.trim(),
            phone: phone.trim(),
            passkey: passkey.trim(),   // 🔑 store passkey
            isActive: true,
            assignedCount: 0,
